...
`git rev-parse HEAD`, sharing the exact hang mode fixed for the campaign hot
path (HANDOFF.md sections 12 and the git_head consolidation). Both now go
through `core.git_head`. These tests pin that: each reads a real repo's HEAD by
file, spawns no git subprocess, and keeps its original sentinel when HEAD cannot
be resolved.
"""
import unittest
