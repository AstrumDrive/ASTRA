...
def cycle_lock_root(root: Path) -> Path:
    ...
    sharing one subscription — the 2.0 development line and production — held
    different locks and could deliberate simultaneously against the same
    account.  Inside a single checkout the lock worked; across lines there was
    no mutual exclusion at all, and the resulting quota contention surfaces as
    operational errors that contaminate any measurement.

    The default is therefore machine-wide.  ``ASTRA_LOCK_ROOT`` overrides it,
    which is also how a collaborator can deliberately isolate a line that uses
    a *different* account: separate credentials, separate lock root.
    """
...
def acquire_cycle_slot(root: Path, max_slots: int = 1) -> tuple:
    ...
    ``root`` is kept for callers and for the ``ASTRA_LOCK_ROOT`` fallback
    below; the lock itself is machine-wide by default (see
    :func:`cycle_lock_root`).
    """
