...
failures encountered while making the system measurable. That second part turned
out to carry most of the engineering content: of roughly eight defects found
during evaluation, six were in the \emph{measurement infrastructure} rather than
in the reasoning engine, and three of four benchmark runs measured the harness
instead of the system. We report a cross-agent instruction injection observed in
72\% of deliberations, a phase-budget allocation that starved the one step that
converts a rejected validator into a result, and an output-token ceiling
consumed entirely by internal reasoning. We also report, without softening it, a
...
This asymmetry is exploited directly in the case study of
Section~\ref{sec:warp}, where an exact witness settles a question at a fraction
of the cost of the global optimisation we had run by hand.
...
\section{Case study: energy conditions of a travelling-wave warp metric}
\label{sec:warp}

A physics question exercised the whole chain. For the $(3\!+\!1)$ ADM metric
\[
ds^{2}=-\alpha^{2}dt^{2}+\psi^{4}\left[(dx+\beta^{x}dt)^{2}+dy^{2}+dz^{2}\right],
\qquad
\alpha=1+\tfrac{f}{4},\;\;\beta^{x}=-\tfrac{f}{2},\;\;\psi=1+\tfrac{f}{5},
\]
with $f=\left[1+(r^{2}/R^{2})^{2}\right]^{-1}$, $\xi=x-vt$, $v=\tfrac12$, the
system formed and decided the claim
$m_{p}=\min_{|\mathbf{s}|=1} 8\pi R^{2}T_{\mu\nu}k^{\mu}k^{\nu}<0$ at the wall
point $p=(0,0,R,0)$.

The reviewer first returned \textsc{revise}, labelling the submitted validator
\code{sampling\_as\_proof}: numerical optimisation proposes candidates, it does
not prove a sign. Only after the author supplied an \emph{exact witness} did the
review pass. The oracle then executed 15 of 15 checks:
\[
\mathbf{s}_{*}=(0,-1,0),\qquad
q(\mathbf{s}_{*})=-\tfrac{23778856}{143496441}<0
\quad\Longrightarrow\quad m_{p_R}<0\ \ \forall R>0 .
\]

Two aspects are worth isolating. First, the \textbf{cross-check}: an
independently written validator, using a different strategy (exact Lagrange
minimisation rather than a witness), had obtained a global minimum of
$-196091158785784/1172859407230599\approx-0.16719$. The witness value
$\approx-0.16571$ lies \emph{above} it by $0.9\%$, which is the only ordering
consistent with both being correct. Two independent implementations agreeing on
a sign and standing in the right relation is worth more than either alone.

Second, the \textbf{scoping}. The system reported
\code{original\_claim\_verdict: INCONCLUSIVE} with the reason that an exact
witness refutes this explicit configuration without excluding others, and marked
goal coverage partial with seven deferred items. The cycle validated its own
conjecture and stated separately that the human's question remained open. That
is the re-anchoring axis of Section~\ref{sec:axes} doing exactly what it was
built for: pruning one branch is useful, negative, and not a closed programme.
...
\item[Negative results are first-class.] Pruning a metric family, exhausting a
...
\textbf{Note on scope.} This report describes an engineering system and its
evaluation. It does not claim novel physics; the case study of
Section~\ref{sec:warp} prunes one metric family and is reported as a negative
result with its scope stated explicitly.
