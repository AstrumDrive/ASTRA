...
established from what is not, and is explicit about one number that does not yet
exist: a head-to-head quality measurement. ASTRA 1.0's only deposited benchmark
data is eight execution smoke runs with zero scientific and zero audit cases, so
the comparison everyone expects to see cannot honestly be printed today. The
measurement is running; this document is written so the numbers can be dropped
into it without rewriting the argument around them.
\end{abstract}
...
\subsection{The head-to-head number does not exist}

The figures usually quoted for ASTRA---false acceptance $0.0$, false rejection
$0.0$, strict accuracy $0.696$, audit recall $1.0$, sound-validator false alarm
$0.0$, over 43 cases---were produced by the \emph{2.0 code} running production's
\emph{configuration}. They measure 2.0.

ASTRA 1.0's own deposited benchmark data is eight runs, all smoke tier, whose
summaries read \code{scientific: \{cases: 0\}} and \code{audit: \{cases: 0\}}.
The only quantity ever measured on that line is execution reproducibility
(verdict accuracy $1.0$, cross-run agreement $1.0$, $n=8$).

\begin{center}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Standard tier, 43 cases} & \textbf{ASTRA 1.0} & \textbf{ASTRA 2.0} \\
\midrule
scientific cases                 & \pending & 23 \\
false acceptance                 & \pending & 0.0 \\
false rejection                  & \pending & 0.0 \\
strict accuracy                  & \pending & 0.696 \\
audit: critical defect recall    & \pending & 1.0 \\
audit: false alarm, sound code   & \pending & 0.0 \\
execution: verdict accuracy      & 1.0 ($n{=}8$, smoke) & 1.0 \\
\bottomrule
\end{tabular}
\end{center}

The measurement is running on the production line with the same suite, tier and
oracle. Two caveats belong with it when it lands: one audit case differs between
the checkouts by a fixture correction, and---more importantly---this benchmark
exercises the atomic cycle only.
