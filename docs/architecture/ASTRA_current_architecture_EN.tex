...
Roles may be reassigned for controlled ablations, but the compact production contract is explicit: Codex CLI with \texttt{gpt-6-astra} at \texttt{xhigh} effort proposes, synthesizes, reviews, and analyzes, with \texttt{gpt-5.6-sol} as the quota fallback; AGY with \texttt{gemini-3.1-pro-high} at \texttt{high} effort is the primary co-proposer, critic, and navigator, with \texttt{gemini-3.8-flash-high} as the first configured fallback and \texttt{gemini-3.7-flash-high} as the second; Claude Code with \texttt{claude-opus-5-5} writes and repairs code, with \texttt{sonnet} as the quota fallback (pinned on 25 September 2026 after probing each rung on the production accounts). The \texttt{scripts/audit\_architecture.py} command verifies this map, the binaries, epistemic gates, local scientific engines, and configured formal routes without reading or printing secrets. \texttt{ASTRA\_REQUIRED\_LOCAL\_ENGINES} makes missing production engines a blocking audit failure. The idempotent \path{scripts/bootstrap_wsl_scientific_stack.ps1} script recreates the pinned Debian stack. Other supported providers are reserved for controlled comparisons.
